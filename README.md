...
- A free [Hugging Face](https://huggingface.co) account (Read token)
...
CookieCreate is hosted on [Render](https://render.com) as a Node.js web service.
...
> **Note:** Render's free tier spins down after 15 minutes of inactivity. The first request after a period of inactivity may be slow (~30s) while the service wakes up. Upgrading to a paid Render plan keeps the service always-on.
...
├── server.js           # Express server — static files, rate limiting, HF proxy
